...
HRESULT CSpeechBasics::StartKinect()
{
    ...
    if (SUCCEEDED(hr))
    {
		...
		pAudioSource->OpenReader(&audioBeamFrameReader);
		pAudioSource->get_SubFrameLengthInBytes(&subFrameLengthInBytes);
    }
...
HRESULT CSpeechBasics::InitializeSpeech()
{
	...
	m_pKinectSensor->get_AudioSource(&pAudioSource);
	pAudioSource->OpenReader(&audioBeamFrameReader);
	pAudioSource->get_SubFrameLengthInBytes(&subFrameLengthInBytes);
	// バッファを作成する

	audioBuffer.resize(subFrameLengthInBytes / sizeof(float));
	//audioFile.Open("mizup" + std::to_string(count) + ".wav");
	count++;
    ...
    while (fetched > 0)
    {
        ...
        case SPEI_RECOGNITION:
            if (SPET_LPARAM_IS_OBJECT == curEvent.elParamType)
            {
				...
				ISpStreamFormat **ppStream =NULL;
				result->GetAudio(1, 1, ppStream);
                if (SUCCEEDED(hr))
                {

					ComPtr<IAudioBeamFrameList> audioBeamFrameList;
					ComPtr<IAudioBeamFrame> audioBeamFrame;
					//audioBeamFrameList->OpenAudioBeamFrame(0, &audioBeamFrame);

					ComPtr<IAudioBeamSubFrame> audioBeamSubFrame;
					//audioBeamFrame->GetSubFrame(0, &audioBeamSubFrame);
                    ...
                    if ((pPhrase->pProperties != NULL) && (pPhrase->pProperties->pFirstChild != NULL))
                    {
                        ...
                        if (pSemanticTag->SREngineConfidence > ConfidenceThreshold)
                        {
							...
							wchar_t buf[1024];
							_snwprintf_s(buf, 1024, _TRUNCATE, L"%s\n", pSemanticTag->pszValue);
							OutputDebugString(buf);
                            m_pTurtleController->DoAction(action);
							if (_tcscmp(pSemanticTag->pszValue,L"かいし") == 0) {
								if (PathFileExists(L"speech.txt")) {
									std::ofstream ofs("speech.txt", std::ios::app);
									ofs << "start" << std::endl;
									OutputDebugString(L"かいし");
								}
								else {
									FILE *fp;
									...
									std::ofstream ofs("speech.txt", std::ios::app);
									ofs << "start" << std::endl;
									OutputDebugString(L"Make");
									fclose(fp);
								}
							}
							if (_tcscmp(pSemanticTag->pszValue, L"さくじょ") == 0) {
								std::ofstream ofs("delete.txt", std::ios::app);
								ofs << "delete" << std::endl;
								OutputDebugString(L"さくじょ");
							}
                        }
						wchar_t buf[1024];
						_snwprintf_s(buf, 1024, _TRUNCATE, L"%s\n", pSemanticTag->pszValue);
...
TurtleAction CSpeechBasics::MapSpeechTagToAction(LPCWSTR pszSpeechTag)
{
    ...
    {
        {L"すすむ", TurtleActionForward},
        {L"さがる", TurtleActionBackward},
        {L"ひだりをむく", TurtleActionTurnLeft},
        {L"みぎをむく", TurtleActionTurnRight}
    };
